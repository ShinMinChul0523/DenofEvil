...
void CRenderTexture::Set_RenderTarget(const UINT& uiNumViews, BOOL bDepthStencilView)
{
...
void CRenderTexture::Set_Texture(const UINT& uiSlot, const UINT uiIndex)
{
										...
										, const DXGI_FORMAT& eFormat, const WORD& wSizeX, const WORD& wSizeY, FLOAT fRenderPosX, FLOAT fRenderPosY)
{
	...
	{
		MSG_BOX("CRenderTarget Ready_RenderTexture Failed");
...
HRESULT CRenderTexture::Ready_RenderTexture(const DXGI_FORMAT& eFormat, const WORD& wSizeX, const WORD& wSizeY, FLOAT fRenderPosX, FLOAT fRenderPosY)
{
	...
	{
		MSG_BOX("RenderTarget - CreateTexture2D Failed");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("RenderTarget - CreateRenderTargetView Failed");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("RenderTarget - CreateShaderResourceView Failed");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("RenderTarget - CreateDepthStencilView Failed");
		return E_FAIL;
	}
...
void CRenderTexture::Release(void)
{
	...
	if (Safe_Com_Release(m_pRTTexture))
		MSG_BOX("m_pRTTexture Release Failed");

	if (Safe_Com_Release(m_pRTView[0]))
		MSG_BOX("m_pRTView Release Failed");

	if (Safe_Com_Release(m_pRTView[1]))
		MSG_BOX("m_pRTView Release Failed");

	if (Safe_Com_Release(m_pShaderResourceView[0]))
		MSG_BOX("m_pShaderResourceView Release Failed");
