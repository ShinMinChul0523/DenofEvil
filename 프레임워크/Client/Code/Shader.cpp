...
CShader* CShader::Create(ID3D11Device* pGraphicDev, ID3D11DeviceContext* pContext, WCHAR* szFileName, BYTE byLayoutFlag)
{
	CShader* pShader = new CShader(pGraphicDev, pContext);

	if (FAILED(pShader->Ready_Shader(szFileName, byLayoutFlag)))
	{
		MSG_BOX("CShader Ready_Shader Failed");
...
HRESULT CShader::Ready_Shader(WCHAR* szFileName, BYTE byLayoutFlag)
{
	...
	{
		MSG_BOX("Load_ShaderFromFile VS Failed");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("CreateInputLayout Failed");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("Load_ShaderFromFile PS Failed");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("CreatePixelShader Failed");
		return E_FAIL;
	}
...
HRESULT CShader::Load_ShaderFromFile(WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
	HRESULT hr = S_OK;
...
void CShader::Release(void)
{
	if (Safe_Com_Release(m_pVertexShader))
		MSG_BOX("m_pVertexShader Release Failed");

	if (Safe_Com_Release(m_pVertexLayout))
		MSG_BOX("m_pVertexLayout Release Failed");

	if (Safe_Com_Release(m_pPixelShader))
		MSG_BOX("m_pPixelShader Release Failed");
