...
CTerrainTex::CTerrainTex(ID3D11Device* pGraphicDev, ID3D11DeviceContext* pContext)
...
CTerrainTex* CTerrainTex::Create(ID3D11Device* pGraphicDev, ID3D11DeviceContext* pContext
	, const WORD& wCntX, const WORD& wCntZ, const WORD& wItv)
{
	CTerrainTex* pTerrainTex = new CTerrainTex(pGraphicDev, pContext);

	if (FAILED(pTerrainTex->Create_Buffer(wCntX, wCntZ, wItv)))
	{
		MSG_BOX("TerrainTex Create_Buffer Failed");
...
CTerrainTex* CTerrainTex::Clone_Resource(void)
{
...
HRESULT CTerrainTex::Create_Buffer(const WORD& wCntX, const WORD& wCntZ, const WORD& wItv)
{
	HRESULT hr = E_FAIL;
...
//	CloseHandle(hFile);
	...
	// Set Collision
	Set_TerrainVtxInCollision(m_pVertex);
